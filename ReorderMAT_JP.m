function [MATindices,MATreordered,MATcost, Costs, Epoch, STD100] = ReorderMAT_JP(MAT,EpochMax,cost)
%REORDERMAT         Reorder matrix for visualization
%
%   [MATindices,MATreordered,MATcost, Costs, Epoch, STD100] =
%   ReorderMAT_JP(MAT,EpochMax,cost);
%
...
STD100=1;   % JP
%for h=1:H
while(STD100>.2 && Epoch<EpochMax) %JP
    Epoch=Epoch+1;
    a = 1:N;
    ...
    % JP{
    Costs(Epoch)=MATcostnew;
    if (Epoch>100)
        STD100=std(Costs((end-100):end));
    end
    % }JP
%    [x,y] = find(MAT(a,a));
