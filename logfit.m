...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (modified JP april-2012)

function [slope, intercept,R2, Chi2, S, extra] = logfit(x,y,varargin)
        ...
        if sum(yMask)>0
            ...
            if sum(yNegMask)>0 % there are proper negative values
                ...
                if sum(y<10)/length(y) < (1/2) % if less than half your data is below than 10.
                    ...
                    % JP warning(['values with y==0 were removed. '...
                    % JP         'Nothing you can really do about it sorry.']);
                end
...
%% Calculate R2 and Chi2 (modified JP april-2012)
% Note that this is done after the data re-scaling is finished.
% R2 = mean((logY-estY).^2);

err = sum((logY-estY).^2);
tot = sum((logY-mean(logY)).^2);
R2 = 1-err/tot;

Chi2 = sum((logY-estY).^2./estY);
